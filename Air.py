...
engine = create_engine("mysql+mysqlconnector://root:Nith_1104@localhost/airbnb") 
...
title_text = '''<h1 style='font-size: 36px;color:#ff5a5f;text-align: center;'>AIRBNB</h1><h2 style='font-size: 24px;color:white;text-align: center;'>Explore Your Dream Stays</h2>'''
...
if selected == "OVERVIEW":
    ...
    st.markdown('''**In the 'Discover' section, users have the opportunity to explore countries through dynamic geo-visualizations,
                supplemented with price insights.**''')              
    ...
    image_path="C:/Users/Nisha Preetha M/Airbnb/airbnb-powerbi.png"
...
if selected == "INSIGHTS":
    ...
    if select_insight =="TOP INSIGHTS":
        ...
        if query==opt[0]:
            ...
            with col1:
                df=pd.read_sql_query('''SELECT h.name, h.country, sub.price
            FROM hotels_info h
            JOIN (
                ...
                LIMIT 10
            ) sub ON h.name = sub.name AND h.price = sub.price''',con=engine)
